...
def checkout():
    ...
    # 1) Берём все позиции корзины у пользователя
    cart_items = db_sess.query(CartItem).filter_by(user_id=user_id).all()
    if not cart_items:
        return redirect(url_for('main_page'))

    # 2) Подхватываем адреса и способы оплаты
    ...
    if request.method == 'POST':
        ...
        # 3) Считаем итоговую сумму по всем variant
        ...
        # 5) Привязываем к заказу каждую позицию
        for ci in cart_items:
            oi = OrderItem(
                order_id    = order.id,
                product_id  = ci.variant.product_id,  # <- берём из variant
                variant_id  = ci.variant_id,
                quantity    = ci.quantity,
                unit_price  = ci.variant.price       # <- тоже из variant
            )
            db_sess.add(oi)
            # и удаляем позицию из корзины
            db_sess.delete(ci)
