...
# Настройки Flask-Mail для порта 465 (SSL)
app.config.update(
...
# Сериализатор токенов для сброса пароля
...
def verify_registration():
    ...
    # на GET — отправляем письмо один раз
...
def forgot_password():
    if request.method == 'POST':
        ...
        # вместо redirect(url_for('login')) делаем так:
...
def profile():
    ...
    addresses = db_sess.query(Address).filter(Address.user_id == user.id).all()
    payments = db_sess.query(Payment).filter(Payment.user_id == user.id).all()
    ...
    if request.method == 'POST':
        ...
        email = email_sub
        phone_sub = request.form.get('phone_num')
        phone = phone_sub
    ...
    open_modal = (request.args.get('modal') == 'payments')
...
def reg():
    if request.method == 'POST':
        ...
        # сохраняем данные и код в сессии
        session['reg_data']       = {
            'first_name': first_name,
            'last_name':  last_name,
            'phone_num':  phone_num,
...
def add_to_favorites(product_id):
    if 'user_id' not in session:
        return redirect(url_for('login'))
    db_sess = db_session.create_session()
    exists = db_sess.query(Favorite)\
                    .filter_by(user_id=session['user_id'], product_id=product_id)\
                    .first()
    if not exists:
        db_sess.add(Favorite(user_id=session['user_id'], product_id=product_id))
        db_sess.commit()
...
def favorites_view():
    ...
    liked_ids = [f.product_id for f in favs]

    products = []
    if liked_ids:
        products = db_sess.query(Product).filter(Product.id.in_(liked_ids)).all()
...
def inject_context():
    db_sess = db_session.create_session() if 'user_id' in session else None

    # корзина
    if db_sess:
        cis = db_sess.query(CartItem).filter_by(user_id=session['user_id']).all()
        cart_items = {ci.product_id: ci.quantity for ci in cis}
...
def remove_from_cart(product_id):
    """Уменьшаем количество на 1 или удаляем."""
    if 'user_id' not in session:
        return redirect(url_for('login'))
    db_sess = db_session.create_session()
    item = db_sess.query(CartItem)\
...
def product_detail(product_id):
    ...
    slides = [product.image_main]
    for attr in ('image_1','image_2','image_3'):
        fn = getattr(product, attr)
        if fn:
            slides.append(fn)
...
@app.route('/product/<int:product_id>/review', methods=['POST'], endpoint='add_review')
def add_review(product_id):
    ...
    for f in request.files.getlist('images'):
        if f and allowed_file(f.filename, ALLOWED_IMG):
            fn = secure_filename(f"{product_id}_{session['user_id']}_{f.filename}")
    ...
    rev = Review(
        ...
        text       = text,
        rating     = rating,
        images     = json.dumps(images),    # в БД — JSON-строка
        video_url  = video_url
    )
    db_sess.add(rev)
    # обновляем счётчики в продукте
    prod = db_sess.get(Product, product_id)
    prod.reviews_cnt = prod.reviews_cnt + 1
    prod.rating = (prod.rating*(prod.reviews_cnt-1) + rating) / prod.reviews_cnt
    db_sess.commit()
...
@app.route("/favorites")
def favorites():
    # TODO: здесь будет ваша логика «Избранного»
    # пока что перенаправляем на главную
    return redirect(url_for('main_page'))
