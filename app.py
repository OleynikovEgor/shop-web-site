...
def profile():
    ...
    # 2) если POST — сохраняем изменения профиля
    if request.method == 'POST':
    ...
    # флаги для открытия модалки оплат и режима редактирования
    open_payment_modal = (request.args.get('modal') == 'payments')
    edit_mode          = (request.args.get('edit')  == 'true')

    # 3) собираем ВСЕ доставленные варианты, чтобы по ним можно было оставить отзыв
    delivered_variants = []
    for order in orders:
        if order.status == 'Доставлен':
            for oi in order.items:
                # убедимся, что одно и то же не добавится дважды
                if oi.variant not in delivered_variants:
                    delivered_variants.append(oi.variant)

    # 4) рендерим шаблон вместе с новым списком delivered_variants
...
def order_detail(order_id):
    ...
    # защита: если такого заказа нет или он не ваш — 404
    if not order or order.user_id != session['user_id']:
        abort(404)

    # простая логика: рассчитываем ожидаемую дату доставки через 3 дня
...
def review_form(variant_id):
    ...
    # Проверяем, что у пользователя есть доставленный заказ с этим вариантом
    delivered = (
    ...
    rev = Review(
        ...
        video_url   = video_fn  # просто имя файла
    )
    db_sess.add(rev)
...
def add_review(variant_id):
    """Сохраняет отзыв — текст, рейтинг, фото и видео-ссылку."""
    if 'user_id' not in session:
        return redirect(url_for('login'))
    db_sess = db_session.create_session()
    user_id = session['user_id']

    # Повторная проверка доставки на случай прямого POST
    delivered = (
        db_sess.query(OrderItem)
               .join(Order)
               .filter(
    ...
    # Обновляем агрегированные поля у варианта
...
def checkout():
    ...
    # 1) Берём все позиции корзины
    cart_items = db_sess.query(CartItem).filter_by(user_id=user_id).all()
    if not cart_items:
        return redirect(url_for('main_page'))

    # 2) Подтягиваем адреса и способы оплаты
    ...
    if request.method == 'POST':
        # 3) Считываем выбранные адрес и способ оплаты
        ...
        db_sess.add(order)
        db_sess.flush()  # чтобы появился order.id

        # 6) Привязываем к заказу каждую позицию и удаляем из корзины
...
def order_confirmation(order_id):
    ...
    # рассчитываем ожидаемую дату доставки через 3 дня
    expected_date = order.created_at + timedelta(days=3)
